...
sh -c "$OM sync --mode github-push" | rg '"ok": true' >/dev/null

# Bootstrap mode should also pass (pull -> reindex -> push)
sh -c "$OM sync --mode github-bootstrap" | rg '"ok": true' >/dev/null
...
sh -c "$OM adapter cred-resolve --ref env://OMNI_TOKEN_TEST --mask" | rg '"ok": true' >/dev/null

# Notion dry-run
sh -c "$OM adapter notion-write --database-id db123 --title 'hello' --content 'world' --dry-run" | rg '"mode": "dry_run"' >/dev/null
sh -c "$OM adapter notion-query --database-id db123 --dry-run" | rg '"mode": "dry_run"' >/dev/null

# R2 dry-run
printf 'abc' > "$TMP_BASE/sample.txt"
sh -c "$OM adapter r2-put --file $TMP_BASE/sample.txt --url 'https://example.com/presigned-put' --dry-run" | rg '"mode": "dry_run"' >/dev/null
sh -c "$OM adapter r2-get --out $TMP_BASE/out.bin --url 'https://example.com/presigned-get' --dry-run" | rg '"mode": "dry_run"' >/dev/null
